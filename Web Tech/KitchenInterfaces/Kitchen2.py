...
# Optional debugging: print the 'type' field if present
...
# -----------------------
# Kitchen Manager (with completed flag and loader)
# -----------------------
class KitchenManager:
    def __init__(self):
        ...
        self.IDX_MONGO_ID = 8  # stores the MongoDB _id (ObjectId) when present

        # orders: [dish, bill_number, remarks, locked(bool), ready(bool), batch_id, timestamp_added, completed(bool), mongo_id]
        self.orders = []
    ...
    # -----------------------
    # MongoDB loader (method)
    # -----------------------
    def load_orders_from_mongodb(self, mongo_records):
        """
        Loads orders from MongoDB records into self.orders and self.batches.
        Expected fields in each record (if present):
            dish, bill_number, remarks, locked, ready, batch_id, timestamp, completed
        This method:
          - reads completed flag if present (defaults False)
          - validates timestamp
          - generates a batch_id if missing and keeps batch_counter coherent
          - avoids duplicate batches
          - stores the MongoDB _id in the order entry (or None)
        """
        for rec in mongo_records:
            ...
            if batch_id is None:
                ...
                # ensure batch_counter is at least as large as the batch_id seen
                try:
                    bid_int = int(batch_id)
                    if bid_int > self.batch_counter:
                        self.batch_counter = bid_int
                    batch_id = bid_int
                except Exception:
                    # if it's not convertible to int, still store but keep counter unchanged
                    pass

            # Keep batches coherent (avoid duplicates)
            exists = any(b[1] == batch_id for b in self.batches)
            if not exists:
                # sanitize timestamp again (store creation time)
                batch_ts = timestamp if isinstance(timestamp, (int, float)) else time.time()
                self.batches.append([dish, batch_id, locked, batch_ts])

            # Append the order record with completed flag respected; store mongo_id
            self.orders.append([
                dish,
                bill,
                remarks,
    ...
    def feed_next_item_to_kitchen(self):
        ...
        # include completed flag (False initially) and mongo_id None (we don't insert here)
        self.orders.append([dish, bill_number, remarks, False, False, batch_id, time.time(), False, None])
        if not items:
            self.bill_queue.popleft()
        return dish

    # Direct dine-in/table order (created locally; per your plan we won't insert to DB)
        ...
        for i in range(len(self.batches) - 1, -1, -1):
            ...
            if b[0] == dish and not b[2]:
                ...
                for o in self.orders:
                    if o[self.IDX_DISH] == dish and o[self.IDX_BATCH] == b[1]:
    ...
    def confirm_batch_done(self, dish, batch_index):
        ...
        for o in self.orders:
            if o[self.IDX_DISH] == dish and o[self.IDX_BATCH] == batch_index and o[self.IDX_LOCKED] and not o[self.IDX_COMPLETED]:
                ...
                updated = True
                # write back to mongodb if this order came from DB
                mongo_id = o[self.IDX_MONGO_ID]
                if mongo_id:
                    try:
                        # ensure ObjectId type when possible
                        collection.update_one({"_id": ObjectId(mongo_id)}, {"$set": {"ready": True}})
                    except Exception:
                        # ignore DB update errors for robustness
                        pass
        return updated
    ...
    def get_unlocked_batches(self):
        """
        Return list of tuples: (dish, latest_unlocked_batch_id, [orders...])
        Only returns batches that have non-completed orders.
        """
        unlocked = []
        seen = set()
        # iterate per-dish, find latest unlocked non-completed batch
        for o in self.orders:
            dish = o[self.IDX_DISH]
            if dish in seen:
                continue
            # find all batch ids for this dish that are unlocked and have non-completed items
            batch_ids = [x[self.IDX_BATCH] for x in self.orders
                         if x[self.IDX_DISH] == dish and not x[self.IDX_LOCKED] and not x[self.IDX_COMPLETED]]
            if not batch_ids:
                seen.add(dish)
                continue
    ...
    def get_locked_batches(self):
        """
        Return list of tuples: (dish, batch_id, [orders...]) for batches that are locked and have non-ready & non-completed orders.
        Ensures only one entry per dish (latest locked batch sequence preserved by iteration over batches).
        """
        locked = []
        added = set()
        for b in self.batches:
            ...
            if dish in added:
                continue
            orders = [x for x in self.orders
                      if x[self.IDX_DISH] == dish and x[self.IDX_BATCH] == b[1] and not x[self.IDX_READY] and not x[self.IDX_COMPLETED]]
            if orders:
    ...
    def get_ready_bills(self):
        """
        Return (dine_in_list, delivery_list) where each is a list of bill identifiers that have all non-completed items marked ready.
        Uses a robust check of bill string to detect table bills.
        """
        dine_in = []
        delivery = []
        ...
        for bill in all_bills:
            ...
            if bill_orders and all(o[self.IDX_READY] for o in bill_orders):
                try:
                    is_table = isinstance(bill, str) and bill.startswith("Table:")
                except Exception:
                    is_table = False
                if is_table:
                    dine_in.append(bill)
        ...
        def table_key(bill):
            if isinstance(bill, str) and bill.startswith("Table:"):
                try:
                    return int(bill.split(":")[1])
                except Exception:
                    return float('inf')
            return float('inf')
    ...
    def clear_completed_and_cleanup_batches(self):
        """
        Remove completed orders and also remove orphaned batches (batches with no remaining orders).
        """
        # remove completed orders
        self.orders = [o for o in self.orders if not o[self.IDX_COMPLETED]]
        # remove batches that have no orders referencing them
        batch_ids_in_orders = set(o[self.IDX_BATCH] for o in self.orders)
        self.batches = [b for b in self.batches if b[1] in batch_ids_in_orders]
...
class KitchenApp(tk.Tk):
    def __init__(self, kitchen_manager: KitchenManager):
        ...
        # timestamp label registry for live updates
        # each entry is (label_widget, batch_id, status_string, created_timestamp)
        self.timestamp_labels = []
        # start the refresher loop
        self._start_timestamp_refresher()
    ...
    def _populate_chef_panels(self):
        ...
        # Clear timestamp registry - we'll re-register visible labels
        ...
        for dish, batch_id, orders in pending_batches:
            ...
            # create a label for the batch info (status + elapsed), register it for updates
            if created_ts:
                secs = int(time.time() - created_ts)
                m, s = divmod(secs, 60)
                elapsed = f"{m:02d}:{s:02d}"
                ts_text = f"Batch #{batch_id}  •  Pending  •  {elapsed}"
            else:
                ts_text = f"Batch #{batch_id}  •  Pending"

            ts_label = ttk.Label(card, text=ts_text, font=("Helvetica", 9))
            ts_label.pack(anchor="w", pady=(2, 6))

            if created_ts:
                # register label for live updates
                self.timestamp_labels.append((ts_label, batch_id, "Pending", created_ts))
        ...
        for dish, batch_id, orders in preparing_batches:
            ...
            if created_ts:
                ...
                m, s = divmod(secs, 60)
                elapsed = f"{m:02d}:{s:02d}"
                ts_text = f"Batch #{batch_id}  •  Preparing  •  {elapsed}"
            else:
                ts_text = f"Batch #{batch_id}  •  Preparing"

            ts_label = ttk.Label(card, text=ts_text, font=("Helvetica", 9))
            ts_label.pack(anchor="w", pady=(2, 6))

            if created_ts:
                self.timestamp_labels.append((ts_label, batch_id, "Preparing", created_ts))
    ...
    def _populate_dinein(self):
        ...
        ready_dishes = [o for o in self.kitchen.orders if o[self.kitchen.IDX_READY] and not o[self.kitchen.IDX_COMPLETED] and isinstance(o[self.kitchen.IDX_BILL], str) and o[self.kitchen.IDX_BILL].startswith("Table:")]
        if not ready_dishes:
        ...
        def table_sort_key(x):
            try:
                ...
                return float('inf')
        for table in sorted(grouped.keys(), key=table_sort_key):
            ...
            for item in grouped[table]:
                ...
                served_btn = ttk.Button(row, text="Mark Completed",
                                        command=lambda it=item: self._serve_item(it))
                served_btn.pack(side="right")

    def _serve_item(self, order_item):
        # mark first matching ready & not completed item as completed
        # order_item is the order list object (so we can update it and access mongo id)
        for o in self.kitchen.orders:
            if o is order_item and o[self.kitchen.IDX_READY] and not o[self.kitchen.IDX_COMPLETED]:
                o[self.kitchen.IDX_COMPLETED] = True
                mongo_id = o[self.kitchen.IDX_MONGO_ID]
                if mongo_id:
                    try:
                        collection.update_one({"_id": ObjectId(mongo_id)}, {"$set": {"completed": True}})
                    except Exception:
                        # ignore DB errors but continue
                        pass
                messagebox.showinfo("Served", f"{o[self.kitchen.IDX_DISH]} for {o[self.kitchen.IDX_BILL]} marked completed.")
                break
        self._refresh_all_pages()
    ...
    def _populate_delivery(self):
        ...
        for bill in delivery_bills:
            ...
            btn = ttk.Button(card, text="Mark Completed", command=lambda b=bill: self._pack_delivery(b))
            ...
            if o[self.kitchen.IDX_BILL] == bill and o[self.kitchen.IDX_READY] and not o[self.kitchen.IDX_COMPLETED]:
                ...
                if mongo_id:
                    try:
                        ...
                        pass
        messagebox.showinfo("Packed", f"{bill} marked as completed.")
        self._refresh_all_pages()
    ...
    def _start_timestamp_refresher(self):
        """Refresh registered batch labels every second."""
        try:
            now = time.time()
            # iterate over a shallow copy in case the list is modified elsewhere
            for entry in list(self.timestamp_labels):
                try:
                    lbl, batch_id, status, created_ts = entry
                    if not created_ts:
                        # no timestamp available, skip
                        continue
                    secs = int(now - created_ts)
                    m, s = divmod(secs, 60)
                    lbl.config(text=f"Batch #{batch_id}  •  {status}  •  {m:02d}:{s:02d}")
                except Exception:
                    # ignore issues updating a particular label
                    continue
        except Exception:
            pass
    ...
    def _clear_all_ready(self):
        # remove orders that have been completed and cleanup orphan batches
        self.kitchen.clear_completed_and_cleanup_batches()
        messagebox.showinfo("Cleared", "All completed orders removed and orphan batches cleaned.")
        self._refresh_all_pages()
...
if __name__ == "__main__":
    ...
    try:
        ...
        kitchen_manager.load_orders_from_mongodb(mongo_records)
    except Exception as e:
        print("Warning: couldn't load from MongoDB:", e)
    # --------------------------------

    # Optional sample (uncomment if you want to see immediate queue behavior)
